...
import {buildCanonicalResearchModel,buildConsistencyDiagnostics,createRenderGeneration,diagnosticsCsv,filterDiagnostics,buildFeatureCoverage,buildFeatureStability,coverageClassSummary,featureCoverageCsv,featureStabilityCsv,featureStabilityWarnings,featureWarningsCsv,filterFeatureCoverage,buildResearchDashboard,buildQualityDetails,buildRaceTrends,buildMonthlyTrends,comparePeriods,filterProblematicHorses,monthlyTrendsCsv,periodComparisonCsv,problematicHorsesCsv,raceTrendsCsv,RESEARCH_RACE_TARGET} from "./research-dashboard.js";
...
function researchDashboardView(){
  const title='<div class="page-title"><span>RESEARCH DASHBOARD PHASE 3</span><h2>研究ダッシュボード</h2><p>Horse ルートの canonical schema から品質・OCR・時系列傾向を分析します。</p></div>';
  ...
  const model=buildCanonicalResearchModel(races),diagnostics=filterDiagnostics(buildConsistencyDiagnostics(model),state.researchDiagnosticFilters),d=buildResearchDashboard(races),details=buildQualityDetails(races),filtered=filterProblematicHorses(details.rows,state.researchQualityFilters),trends=buildRaceTrends(races),monthly=buildMonthlyTrends(trends),comparison=comparePeriods(trends,state.researchPeriods),value=(x,digits=1)=>x==null?"-":Number(x).toFixed(digits);
